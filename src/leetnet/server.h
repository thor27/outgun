...
public:
    ...
    virtual void setDataCallback(dataCallbackT* fn) throw () = 0;
    virtual void setLagStatusCallback(lagStatusCallbackT* fn) throw () = 0;
    virtual void setPingResultCallback(pingResultCallbackT* fn) throw () = 0;
    ...
    virtual int set_client_timeout(int lagtime, int droptime) throw () = 0;

    //set serverinfo string
    virtual void set_server_info(const char *info) throw () = 0;

    //start up the server at given port
    virtual int start(int port) throw () = 0;
    ...
    virtual int stop(int disconnect_clients_timeout) throw () = 0;
    ...
    virtual int disconnect_client(int client_id, int timeout, uint8_t reason, bool fromUserThread = true) throw () = 0;
...
// server factory
server_c *new_server_c(int thread_priority, int minLocalPort = 0, int maxLocalPort = 0) throw ();
